...
export const analyzeTrainingData = async (data: TrainingMetric[]): Promise<string> => {
  ...
  try {
    // DO NOT use globalThis.process; access process.env.API_KEY directly as per guidelines.
    // We assume process.env.API_KEY is pre-configured and accessible in the execution context.
    ...
    // Access the text property directly (not a method) as per Gemini API guidelines.
